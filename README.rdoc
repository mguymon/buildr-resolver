...
The +Buildr::Resolver.resolve( deps )+ is what takes the activemq dependency and resolves 
all the additional dependencies for it. If you added a +puts+ +dep+ after +Buildr::Resolver.resolve+, it would output:
...
You can also manually exclude dependencies as an optional parameter to +Buildr::Resolver.resolve+. The exclusions
are regexp matched, so you can pass in only a groupId or the full dependency notation.
...
example +buildfile+ with exclusions
    ...
    Java.load # must be called before Buildr::Resolver.resolve if using Ruby due to Rjb startup of JVM by Buildr
    deps = ['org.apache.activemq:activemq-spring:jar:5.4.2']
    <strong>excluded = ['commons-logging', 'log4j', 'logkit' ]</strong>
    deps =  Buildr::Resolver.resolve( deps, <strong>excluded</strong> )
    define 'example' do
        project.version = '0.1.0'
        compile.with deps
        package :jar
